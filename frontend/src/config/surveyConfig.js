export const surveyJson = {
  ...
  pages: [
    ...
    {
      name: "resumen_total",
      title: "RESUMEN GENERAL Y CALIFICACIÓN",
      elements: [

        {
          type: "html",
          name: "nivel_general_html",
          html: `
        <h3>Nivel de Madurez General</h3>
        <p><b>Valor promedio:</b> <span id="valor_general"></span></p>
        <p><b>Significado:</b> <span id="significado_general"></span></p>
        <p><b>Definición:</b> <span id="definicion_general"></span></p>
        <p><b>Ponderación asignada:</b> <span id="ponderacion_general"></span></p>
      `
        },

        {
          type: "html",
          name: "nivel_categorias_html",
          html: `
        <h3>Nivel de Madurez por Categoría</h3>
        <div id="categorias_nivel"></div>
      `
        },

        {
          type: "html",
          name: "grafico_categorias",
          html: "<h3>Gráfico por Categorías</h3><canvas id='graficoCategorias' style='width:100%; height:400px;'></canvas>"
        },

        {
          type: "html",
          name: "grafico_subcategorias",
          html: "<h3>Gráfico por Subcategorías</h3><canvas id='graficoSubcategorias' style='width:100%; height:400px;'></canvas>"
        }
      ],
      visibleIf: "true",
      onValueChanged: function (survey, options) {

        const categorias = {
          "Estratégico": ["GESTION_ESTRATEGICA_SEL"],
          "Administración de Archivos": ["GESTION_DOC_PROC_SEL", "GESTION_DOC_CANAL_SEL"],
          "Procesos de la Gestión Documental": ["SISTEMA_INFO_CORP_SEL", "MODELO_REQ_SEL", "SISTEMA_GDEA_SEL", "DIGITALIZACION_SEL", "ESQUEMA_METADATOS_SEL", "SISTEMA_PRESERVACION_SEL", "ALMACENAMIENTO_NUBE_SEL", "REPOSITORIOS_DIGITALES_SEL"],
          "Tecnológico": ["ARTICULACION_POLITICAS_SEG_INFO_SEL", "COPIA_SEGURIDAD_ARCHIVO_DIG_SEL", "INTEROPERABILIDAD_POLITICO_LEGAL_SEL", "INTEROPERABILIDAD_SEMANTICO_SEL", "INTEROPERABILIDAD_TECNICO_SEL"],
          "Cultural": ["PROGRAMA_GESTION_CONOCIMIENTO_SEL", "MEMORIA_INSTITUCIONAL_SEL", "ARCHIVOS_HISTORICOS_SEL", "REDES_CULTURALES_SEL", "RENDICION_CUENTAS_SEL", "MECANISMOS_DIFUSION_SEL", "ACCESO_CONSULTA_INFO_SEL", "PLAN_AMBIENTAL_SEL"]
        };

        const niveles = [
          { nombre: "Inicial", min: 0, max: 0, definicion: "Entidad carece del producto", ponderacion: "0%" },
          { nombre: "Básico", min: 1, max: 65, definicion: "La entidad está desarrollando el producto teniendo en cuenta necesidades administrativas, legales, funcionales y tecnológicas", ponderacion: "<=65" },
          { nombre: "Intermedio", min: 66, max: 79, definicion: "La entidad implementa el producto teniendo en cuenta necesidades administrativas, legales, funcionales y tecnológicas", ponderacion: ">=66 y <=79" },
          { nombre: "Avanzado 1", min: 80, max: 94, definicion: "La evidencia observada cumple a cabalidad con el ítem; actividades ejecutadas conforme a lo planeado con evidencias documentales", ponderacion: ">=80 y <=94" },
          { nombre: "Avanzado 2", min: 95, max: 100, definicion: "La entidad realiza procesos de mejora continua a los productos teniendo en cuenta necesidades administrativas, legales, funcionales y tecnológicas", ponderacion: ">=95 y <=100" }
        ];
        ...
        const calcularPromedio = (lista) => {
          ...
          let count = 0;
          lista.forEach(item => {
            const val = parseInt(survey.getValue(item) || 0);
            total += val;
            count++;
          });
        ...
        const obtenerNivel = (valor) => niveles.find(n => valor >= n.min && valor <= n.max) || niveles[0];


        const valoresCat = {};
        const nivelesCat = {};
        Object.keys(categorias).forEach(cat => {
          const promedio = calcularPromedio(categorias[cat]);
          valoresCat[cat] = promedio;
          nivelesCat[cat] = obtenerNivel(promedio);
        });


        const valorGeneral = Math.round(Object.values(valoresCat).reduce((a, b) => a + b, 0) / Object.values(valoresCat).length);
        const nivelGeneral = obtenerNivel(valorGeneral);


        document.getElementById("valor_general").innerText = valorGeneral;
        document.getElementById("significado_general").innerText = nivelGeneral.nombre;
        document.getElementById("definicion_general").innerText = nivelGeneral.definicion;
        document.getElementById("ponderacion_general").innerText = nivelGeneral.ponderacion;

        // Categorías
        const divCat = document.getElementById("categorias_nivel");
        divCat.innerHTML = "";
        Object.keys(valoresCat).forEach(cat => {
          divCat.innerHTML += `
        <b>${cat}:</b> Valor: ${valoresCat[cat]} - Significado: ${nivelesCat[cat].nombre} - Definición: ${nivelesCat[cat].definicion} - Ponderación: ${nivelesCat[cat].ponderacion} <br/>
      `;
        });
        ...
        if (window.Chart) {
          ...
          if (ctxCat) new Chart(ctxCat, {
            type: "bar",
            data: {
              labels: Object.keys(valoresCat),
              datasets: [{
                label: "Valor por Categoría",
                data: Object.values(valoresCat),
        ...
        Object.keys(categorias).forEach(cat => {
          categorias[cat].forEach(sub => {
            labelsSub.push(sub);
            dataSub.push(parseInt(survey.getValue(sub) || 0));
          });
        });
        if (window.Chart) {
          const ctxSub = document.getElementById("graficoSubcategorias");
          if (ctxSub) new Chart(ctxSub, {
            type: "bar",
            data: {
              labels: labelsSub,
              datasets: [{
                label: "Valor por Subcategoría",
                data: dataSub,
